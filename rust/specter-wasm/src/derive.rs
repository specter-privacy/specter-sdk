...
use k256::{FieldBytes, NonZeroScalar, ProjectivePoint, PublicKey, Scalar, SecretKey, U256};
...
const DOMAIN_STEALTH_TWEAK: &[u8] = b"SPECTER_STEALTH_TWEAK_V1";
...
fn tweak_scalar(shared_secret: &[u8]) -> Scalar {
    ...
    loop {
        let digest = shake256(DOMAIN_STEALTH_TWEAK, &input, 32);
        let mut field = FieldBytes::default();
        field.copy_from_slice(&digest);
        let t = <Scalar as Reduce<U256>>::reduce_bytes(&field);
        if t.to_bytes().iter().any(|&b| b != 0) {
            input.zeroize();
            return t;
        }
        // Astronomically unlikely: rehash the digest and try again.
        input = digest;
    }
}
